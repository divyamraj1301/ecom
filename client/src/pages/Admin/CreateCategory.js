...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const { data } = await axios.post(`${serverAPI}/category/create-category`, { name })
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault()
    try {
      const { data } = await axios.put(`${serverAPI}/category/update-category/${selected._id}`, { name: updatedName })
      if (data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(`${serverAPI}/category/delete-category/${pId}`)
      if (data.success) {
