...
const HomePage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true)
      const { data } = await axios.get(`${serverAPI}/product/product-list/${page}`)
      setLoading(false)
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(`${serverAPI}/product/product-count`)
      setTotal(data?.total)
    } catch (error) {
      console.log(error)
    }
              ...
              <div className="card m-2" style={{ width: "18rem" }}>
                ...
                <div className="card-body">
                  ...
                  <div className='card-buttons'>
                    ...
                    <button className="btn btn-secondary ms-1 mb-2" onClick={() => {
                      setCart([...cart, p])
