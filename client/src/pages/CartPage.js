...
const CartPage = () => {
    ...
    const getToken = async () => {
        try {
            const { data } = await axios.get(`${serverAPI}/product/braintree/token`)
    ...
    const handlePayment = async () => {
        try {
            ...
            const { data } = await axios.post(`${serverAPI}/product/braintree/payment`, {
                nonce, cart,
            })
            setLoading(false)
                                ...
                                cart?.map((p) => (
                                    <div className="row mb-2 card flex-row" key={p._id}>
                                        <div className="col-md-4">
                                            <img src={`${serverAPI}/product/product-photo/${p._id}`} className="card-img-top" alt={p.name} width="100px" height="100px" />
                                        </div>
                                        <div className="col-md-4">
                                            <p>{p.name}</p>
